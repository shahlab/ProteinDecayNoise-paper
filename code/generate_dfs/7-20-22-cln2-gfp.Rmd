...
---
---
title: "7-20-22 cln2-gfp pup1-rfp decay"
output: html_notebook
---
...
gfp_imaris <- dir("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/imaris/gfp", full.names = TRUE)

#getting the information for imageJ image info files
#getting all the image info txt for all the images in the experiment. 
list.image.info <- list.files("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/image_info_imageJ" , full.names = TRUE)
...
```{r}
dapi_pup1_imaris_cells <- dir("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/imaris/surface/cells", full.names = TRUE) 
...
list.of.files.dapi.pup1 <- list.of.files.dapi.pup1 %>%
    mutate(timepoint = ifelse(timepoint == 3,32,timepoint))
...
```{r}
pup1_surface_imaris <- dir("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/imaris/surface/pup1", full.names = TRUE) 
...
bg <- dir(path = "Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/background" , full.names = TRUE)
...
```{r}
s1.3.4.5.6.7.8.gfp.bg_files <- list.files("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/background/bg_gfp_s1_s8", full.names = T)
...
```{r}
s1.3.4.5.6.7.8.rfp.dapi <- list.files("Imaris-data/2022/pup1_rfp/7-20-22-pup1/cln2/background/bg_mcherry_dapi_s1_s8", full.names = T)
...
```{r}
new.gfpLiveCells %>% 
  write_csv(.,path = "pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data/raw_gfp_int_all.csv")
```
...
```{bash}
mkdir pup1-rfp-gfp-decay/7-20-22-cln2-gfp
mkdir pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data
```


#Saving the CSV with cellular attributtes like area, DNA stain etc 
```{r}
...
write_csv(cell_atributes_final,file = "pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data/7-20-22_cln2_pup1_attributes.csv")
  

```

#saving the CSV with the timeseries data
```{r}
...
write_csv(gfp_cln2Pest_filtered , file = "pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data/gfp_cln2Pest_pup1_filtered.csv")
```
...
```{r}
write_csv(df_for_MechanisticModel , file = "pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data/gfp_cln2Pest_pup1_raw_data.csv")
```
