...
---
---
title: "2.5uM control of the 4-1-22-PI experiment"
output: html_notebook
---
This experiment has 4 sets of data: 
Leu2::GFP-mODC with Pup1-tDimer
0.1% 2.5uM treatment for 30mins (2.5 hrs after galactose media dilution)
...
this RMD is to generate the df for decay rate estimation for the 2.5uM MG132 conc. 
...
```{bash}
mkdir ../../proteasome_inhibition/data/4-1-22/2.5uM
```
...
gfp_imaris <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/imaris/gfp", full.names = TRUE)

#getting the information for imageJ image info files
#getting all the image info txt for all the images in the experiment. 
list.image.info <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/image_info_imageJ" , full.names = TRUE)
...
```{r}
dapi_pup1_imaris_cells <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/imaris/surface/cells", full.names = TRUE) 
...
```{r}
pup1_surface_imaris <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/imaris/surface/pup1", full.names = TRUE) 
...
bg <- dir(path = "../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/background" , full.names = TRUE)
...
```{r}
s1_s2_s5_bg_rfp.dapi <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/background/bg_mcherry_dapi_s1_s2_s5", full.names = T)

rfp.bg.mean.s1.s2.s5 <- lapply(s1_s2_s5_bg_rfp.dapi, function(a){
  read.csv(a) %>% 
...
rfp.dapi.bg.means$`20min` <- rfp.dapi.bg.means$`20min` %>% 
  left_join(.,rfp.bg.mean.s1.s2.s5, by = c("field", "timepoint")) %>% 
...
```{r}
s1_s2_s5_bg_files <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/2.5uM/background/bg_gfp_s1_s2_s5", full.names = T)

gfp.bg.mean.s1.s2.s5 <- lapply(s1_s2_s5_bg_files, function(a){
  read.csv(a) %>% 
  mutate(field = str_split(Label, "_", simplify = T)[,ncol(str_split(Label, "_", simplify = T))-2],
         timepoint = as.numeric(str_remove(str_split(Label, "_", simplify = T)[,ncol(str_split(Label, "_", simplify = T))-1], "t"))) %>% 
  select(field,
...
gfp.bg.mean$`20min` <- gfp.bg.mean$`20min` %>% 
  left_join(.,gfp.bg.mean.s1.s2.s5, by = c("field", "timepoint")) %>% 
...
gfp_rfp_dapi_bgsubAbvZero_tp1  %>%
  select(dapi.mean.bg.sub, timepoint, field, unique.trackID) %>%
  filter(!(unique.trackID == "1000000019_20min_s3")) %>% 
  pivot_wider(values_from = dapi.mean.bg.sub, names_from = timepoint) %>% 
  ggplot(.,aes(x = `1`, y = `31`))+
  geom_point()+
...
```{r}
new.gfpLiveCells %>% 
  write_csv(.,path = "../../proteasome_inhibition/data/4-1-22/2.5uM/raw_gfp_int_all.csv")
```
...
write_csv(cell_atributes_final,file = "../../proteasome_inhibition/data/4-1-22/2.5uM/4-1-22-GFP_mODC_2.5uMMG132_attributes.csv")
...
write_csv(gfp_mODCPest_filtered , file = "../../proteasome_inhibition/data/4-1-22/2.5uM/gfp_mODCPest_2.5uMMG132_filtered.csv")
```
...
```{r}
write_csv(df_for_MechanisticModel , file = "../../proteasome_inhibition/data/4-1-22/2.5uM/gfp_mODCPest_2.5uMMG132_raw_data.csv")
```
