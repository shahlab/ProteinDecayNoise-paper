...
---
---
title: "DMSO control of the 4-1-22-PI experiment"
output: html_notebook
---
This experiment has 4 sets of data: 
Leu2::GFP-mODC with Pup1-tDimer
0.1% DMSO treatment for 30mins (2.5 hrs after galactose media dilution)
...
gfp_imaris <- dir("~/Imaris-data/2022/proteasome_inhibition/4-1-22-PI/DMSO/imaris/gfp", full.names = TRUE)

#getting the information for imageJ image info files
#getting all the image info txt for all the images in the experiment. 
list.image.info <- list.files("~/Imaris-data/2022/proteasome_inhibition/4-1-22-PI/DMSO/image_info_imageJ" , full.names = TRUE)
...
```{r}
dapi_pup1_imaris_cells <- dir("~/Imaris-data/2022/proteasome_inhibition/4-1-22-PI/DMSO/imaris/surface/cells", full.names = TRUE) 
...
```{r}
pup1_surface_imaris <- dir("~/Imaris-data/2022/proteasome_inhibition/4-1-22-PI/DMSO/imaris/surface/pup1", full.names = TRUE) 

pup1_puncta <- get.imaris.info.dapi_pup1.surface(dir.names = pup1_surface_imaris, 
                                                 files_to_read = FilesToReadDAPIRFP) 

pup1_puncta %>% filter(field == "s4")
```
...
bg <- dir(path = "~/Imaris-data/2022/proteasome_inhibition/4-1-22-PI/DMSO/background" , full.names = TRUE)
...
gfp.bg.sub <- sub.bg.intensity.gfp(list.of.files = list.of.files.gfp, bg.means = gfp.bg.mean$`20min` %>% rename("avg.gfp.bg" = "Mean_gfp"))
gfp.bg.sub
...
```{r}
new.gfpLiveCells %>% 
  write_csv(.,path = "~/plots/proteasome_inhibition/data/4-1-22/DMSO/raw_gfp_int_all.csv")
```
...
#blurry cells at timepoint 1 and 2 for s7. There were alot of cells moving in the fields s5 and s6. check the track info (straightness, displacement lenght etc)

```{r}
new.gfpLiveCells %>% 
  filter(field %in% c("s5","s6")) %>% 
  group_by(unique.trackID) %>% 
  mutate(gfp.ratio = gfp.mean.bg.sub[1]/gfp.mean.bg.sub) %>% 
  filter(gfp.ratio > 1) %>% 
  ggplot(.,aes(timepoint, gfp.mean.bg.sub, group = unique.trackID ,color = sample))+
  geom_line(alpha = 0.2)+
  facet_wrap(~field, scales = "free_y")
...
write_csv(cell_atributes_final,file = "/home/Das/plots/proteasome_inhibition/data/4-1-22/DMSO/4-1-22-GFP_mODC_DMSO_attributes.csv")
  
read.csv("/home/Das/plots/proteasome_inhibition/data/4-1-22/DMSO/4-1-22-GFP_mODC_DMSO_attributes.csv")
```
...
write_csv(gfp_mODCPest_filtered , file = "~/plots/proteasome_inhibition/data/4-1-22/DMSO/gfp_mODCPest_DMSO_filtered.csv")
```
...
```{r}
write_csv(df_for_MechanisticModel , file = "~/plots/proteasome_inhibition/data/4-1-22/DMSO/gfp_mODCPest_DMSO_raw_data.csv")
```
