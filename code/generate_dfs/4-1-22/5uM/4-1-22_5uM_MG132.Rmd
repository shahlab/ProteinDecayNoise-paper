...
---
---
title: "5uM control of the 4-1-22-PI experiment"
output: html_notebook
---
This experiment has 4 sets of data: 
Leu2::GFP-mODC with Pup1-tDimer
0.1% 5uM treatment for 30mins (2.5 hrs after galactose media dilution)
...
this RMD is to generate the df for decay rate estimation for the 5uM MG132 conc. 
...
```{bash}
mkdir ../../proteasome_inhibition/data/4-1-22/5uM
```
...
gfp_imaris <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/imaris/gfp", full.names = TRUE)

#getting the information for imageJ image info files
#getting all the image info txt for all the images in the experiment. 
list.image.info <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/image_info_imageJ" , full.names = TRUE)
...
```{r}
dapi_pup1_imaris_cells <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/imaris/surface/cells", full.names = TRUE) 
...
```{r}
pup1_surface_imaris <- dir("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/imaris/surface/pup1", full.names = TRUE) 
...
```{r}
#this should have 6826 no. of cells 
...
#this has 6831 cells. some cells have multiple puncta. maybe just drop the cells. 
```
...
cell_atributes %>% 
  # filter(unique.trackID == "1000001774_20min_s3")
    ...
    dplyr::filter(n > 1L)
#6440 cells. There are 4 cells with multiple pup1 puncta. maybe drop these cells just for simplicity purposes. 
```
...
bg <- dir(path = "../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/background" , full.names = TRUE)
...
gfp.bg.mean$`20min` %>% 
  arrange(timepoint) %>% split(.$field)
```

s2

rfp and dapi bg for s1, s2, and s5
```{r}
s2_bg_rfp.dapi <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/background/bg_mcherry_dapi_s2", full.names = T)

rfp.bg.mean.s2 <- lapply(s2_bg_rfp.dapi, function(a){
  read.csv(a) %>% 
...
rfp.dapi.bg.means$`20min` <- rfp.dapi.bg.means$`20min` %>% 
  left_join(.,rfp.bg.mean.s2, by = c("field", "timepoint")) %>% 
...
```{r}
s2_bg_files <- list.files("../../imaris-data-PI/2022/proteasome_inhibition/4-1-22-PI/5uM/background/bg_gfp_s2", full.names = T)

gfp.bg.mean.s2 <- lapply(s2_bg_files, function(a){
  read.csv(a) %>% 
  mutate(field = str_split(Label, "_", simplify = T)[,ncol(str_split(Label, "_", simplify = T))-2],
         timepoint = as.numeric(str_remove(str_split(Label, "_", simplify = T)[,ncol(str_split(Label, "_", simplify = T))-1], "t"))) %>% 
  select(field,
...
gfp.bg.mean$`20min` <- gfp.bg.mean$`20min` %>% 
  left_join(.,gfp.bg.mean.s2, by = c("field", "timepoint")) %>% 
...
gfp_rfp_dapi_bgsubAbvZero_tp1  %>%
  select(dapi.mean.bg.sub, timepoint, field, unique.trackID) %>%
  filter(!(unique.trackID %in% c("1000000176_20min_s7",
                                 "1000000087_20min_s8",
                                 "1000000118_20min_s8",
                                 "1000000167_20min_s8"))) %>% 
  pivot_wider(values_from = dapi.mean.bg.sub, names_from = timepoint) %>% 
...
deadCell_boundry <- dapi.threshold(df_for_threshold = deadcell_temp_df , lower_lim = 50, uppr_lim = 100 ) %>% 
...
```{r}
new.gfpLiveCells %>% 
  write_csv(.,path = "../../proteasome_inhibition/data/4-1-22/5uM/raw_gfp_int_all.csv")
```
...
gfp.min.int <- gfp.thershold.31tp(
  list.of.files.bg.sub.above.0 = new.gfpLiveCells %>% filter(!(field == "s2" )) %>% 
    split(.$experiment))
...
final.data.31tp.gfp %>% filter(timepoint ==1) %>% 
  split(.$exp.field)
```

```{r}
final.data.31tp.gfp %>% 
  ggplot(.,aes(timepoint, gfp.mean.bg.af.sub.new, group = 
unique.trackID))+
  geom_line(alpha = 0.2)+
...
```
#saving the cellular attributes at t=0
```{r}
...
write_csv(cell_atributes_final,file = "../../proteasome_inhibition/data/4-1-22/5uM/4-1-22-GFP_mODC_5uMMG132_attributes.csv")
...
write_csv(gfp_mODCPest_filtered , file = "../../proteasome_inhibition/data/4-1-22/5uM/gfp_mODCPest_5uMMG132_filtered.csv")
```
...
```{r}
write_csv(df_for_MechanisticModel , file = "../../plots/proteasome_inhibition/data/4-1-22/5uM/gfp_mODCPest_5uMMG132_raw_data.csv")
```
