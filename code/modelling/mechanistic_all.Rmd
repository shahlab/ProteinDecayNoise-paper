...
#1 parameter model with a constant maturation rate: 
 mechanistic.fn.wof.cntDM<- function(par, df){
   ...
   dy <- par[1]
   dm <- ((0.28+0.18 +1.2+0.13)/4 *10^-3)*60
...
```{r}
#5/15/23
#without the stable GFP and without proteasome inhibition
list.of.cells.1 <- bind_rows(list.of.cells) %>% 
  filter(treatment == "none") %>% 
  filter(degron %in% c("mODC","mODC.2","cln2.3","cln2.4","cln2","cln2.2")) %>% 
  split(.$cell.id)
...
single.cell.dy.f.dm <- mclapply(list.of.cells.1, function(a){ 
   ...
   optimx(par = par.optim, 
          ...
          upper = c(0.1, 1, Inf), #changed from dy = 6 to dy = 10 and now to dy = 1
          df = df,
          itnmax = 100000)
   }, mc.cores = 40)
...
converged.cells %>% 
  ...
  summarise(dm.avg = median(dm))

converged.cells %>% 
  group_by(treatment, red, degron) %>% 
  summarise(f.avg = median(f),
            dy.avg = median(dy),
            dm.med = median(dm) )
```
