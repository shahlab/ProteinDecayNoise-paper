...
set the working dir
```{r}
setwd(dir = "/data/sukanya/paper") #change to where the repo is cloned
```
...
##### For GRC 2023 and for the paper
only one set of experiment for each GFP (each contains 3 biologiical replicates)
```{r}
#getting the median value of half lives for each GFP keeping only one set of experiments each containing three biological replicates
...
## Panel D : cv of t.half
##### using all the replicates
...
## Panel E
gfp int cv
#### only three replicates for each GFP
```{r}
...
###fig 2: 
with only three replicates for each
```{r}
right.bot.sec <- ((cvtHalf.plt.grc+plot_spacer()+cvGFP.plt.grc))+
                plot_layout(widths = c(0.5,-0.1,0.5))

right.sec <- (GFPtrace.pup1Rep2022.grc/
                pup1RFP.halfLives.oneReps.grc/right.bot.sec)+
...
figGFP.timelapse <-  (left.sec+plot_spacer()+right.sec)+ 
  plot_layout(widths = c(0.4,-0.05,0.6))
