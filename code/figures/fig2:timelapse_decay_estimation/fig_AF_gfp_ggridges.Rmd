...
```{r}
raw.int.mODC <- read.csv("~/plots/pup1-rfp-gfp-decay/6-28-22-modc-gfp/data/raw_gfp_int_all.csv") %>% 
  dplyr::select(unique.trackID, gfp.mean.bg.sub,timepoint,experiment,exp.field, field, sample, ) %>% 
  mutate(degron = "yeGFP-mODC")

raw.int.cln2 <- read.csv("~/plots/pup1-rfp-gfp-decay/7-20-22-cln2-gfp/data/raw_gfp_int_all.csv") %>%  
  dplyr::select(unique.trackID, gfp.mean.bg.sub,timepoint,experiment,exp.field, field, sample) %>% 
  mutate(degron = "yeGFP-CLN2")

raw.int.cln2.2 <- read.csv("~/plots/pup1-rfp-gfp-decay/10-7-22-cln2-gfp/data/raw_gfp_int_all.csv") %>% 
  dplyr::select(unique.trackID, gfp.mean.bg.sub,timepoint,experiment,exp.field, field, sample) %>% 
  mutate(degron = "yeGFP-CLN2")
```
...
#plot
```{r}
GFP.af.pos.ggrig.plt <- bind_rows(raw.int.mODC, raw.int.cln2, raw.int.cln2.2) %>% 
