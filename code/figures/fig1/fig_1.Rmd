...
#distribution of the AICs 
```{r}
read_csv("~/plots/all_data/aic.csv") %>%
  filter(value < 0.1 , red == "pup1-rfp") %>% 
...
aic.df %>% 
  left_join(.,all.exp.list %>% filter(image.no ==1) %>% select(-delta.time), by = c("cell.id", "red","degron","treatment","exp.field")) %>% 
  filter(degron == "stable.2" ) %>% 
  filter(cell.id == "1000001840_20min_s3_stable.2_pup1-rfp_none")
...
fig1c.df <- all.exp.list %>% 
  ...
  filter(cell.id %in% c( "1000002011_20min_s3_cln2.3_pup1-rfp_none", 
                         ...
                         # "1000002119_20min_s7_stable.2_pup1-rfp_none"
                         )) %>% 
...
fig1c.allcells.df <- fig1c.allcells.df %>% 
  mutate(colony = case_when(exp.field %in% c("20min_s3", "20min_s4") ~ "Replicate 1", 
                            exp.field %in% c("20min_s5", "20min_s6") ~ "Replicate 2",
                            exp.field %in% c("20min_s7", "20min_s8") ~ "Replicate 3"), 
         degron = case_when(degron == "cln2.3" ~ "yeGFP-CLN2",
                            degron == "mODC.2" ~ "yeGFP-mODC",
                            degron == "stable.3" ~ "yeGFP"),
         degron = factor(degron , levels = c("yeGFP-mODC","yeGFP-CLN2","yeGFP")))
...
GFPtrace.pup1Rep2022 <- fig1c.allcells.df %>% 
  ...
  geom_text(data = fig1c.pred %>% 
              ...
              filter(delta.time == 0 , 
                     ...
                     degron == "GFP-mODC") %>% 
              slice(rep(1:n(), 2)) %>% 
              mutate(delta.time = c(1200,1200), 
                     gfpMeanBgAFsub =c(1100,950)), 
...
fig1c.allcells.df %>% 
  ...
  ggplot(.,aes(x = delta.time/60, y = I0_It, group = cell.id))+
  geom_line(alpha = 0.2, color = "gray")+
  geom_line(data = fig1c.pred %>% 
              mutate(dm = ifelse(is.na(dm), Inf, dm)) %>%
              group_by(cell.id) %>% 
              ...
              mutate(I0_It = gfpMeanBgAFsub/gfpMeanBgAFsub[1]) %>% 
              select(-value) %>% 
              pivot_longer(cols = c("I0_It","pred.value")), 
...
fig1c.allcells.df %>% 
  ...
  geom_line(data = fig1c.pred %>% 
              mutate(dm = ifelse(is.na(dm), Inf, dm)) %>%
              group_by(cell.id) %>% 
              filter(ifelse(dm > 1 , model %in% c("dy.f","exponential"), model == "dy.dm")) %>%
              mutate(I0_It = gfpMeanBgAFsub/gfpMeanBgAFsub[1]) %>% 
              select(-value) %>% 
              pivot_longer(cols = c("I0_It","pred.value")), 
            aes(x = delta.time/60, y = value, color = name, group = name),
...
```{r}
SinCellGFPTrace <- fig1c.pred %>% 
  group_by(cell.id) %>% 
  filter(model == "dy.dm.f") %>% 
  select(cell.id, delta.time, gfpMeanBgAFsub, degron, model, colony, pred.gfp) %>% 
  pivot_longer(cols = c("gfpMeanBgAFsub", "pred.gfp")) %>% 
  ggplot(.,aes(x = delta.time/60, y = value, color = name ))+
  geom_line()+
  geom_point(data = fig1c.pred %>% 
               group_by(cell.id) %>% 
               filter(model == "dy.dm.f") ,aes(x = delta.time/60, y = gfpMeanBgAFsub) , color = "green4", size = 0.5)+
...
#fig 1d
```{r}
pup1RFP.halfLives <- aic.df %>% 
  filter(red == "pup1-rfp", treatment == "none", degron %in% c("cln2.3","cln2.2","mODC","mODC.2", "stable","stable.2") ) %>% 
  filter(ifelse(degron %in% c( "stable.2", "stable"), dy < 0.1, dy < 0.5)) %>% 
  group_by(cell.id) %>% 
...
#For the modification of the fig 1d
...
# aic.df.allreps.all <- aic.df %>% 
#   filter(red == "pup1-rfp", treatment == "none", 
#          degron %in% c("cln2.3","cln2.2","cln2","mODC","mODC.2", "stable","stable.2") ) %>% 
#   filter(ifelse(degron %in% c( "stable.2", "stable"), dy < 0.1, dy < 0.5)) %>% 
#   group_by(cell.id) %>% 
#   mutate(dm = ifelse(is.na(dm), Inf, dm)) %>% filter(dm > 0.00001) %>% 
#   filter(ifelse(degron %in% c("mODC","mODC.2"), model == "dy.dm", model == "exponential")) %>% 
#   mutate(colony = case_when(degron == "cln2.2" & colony == "Replicate 1"~ "Replicate 4",
#                             degron == "cln2.2" & colony == "Replicate 2"~ "Replicate 5",
#                             degron == "cln2.2" & colony == "Replicate 3"~ "Replicate 6",
#                             degron == "cln2" & colony == "Replicate 1"~ "Replicate 7",
...
aic.df.allreps.all %>% group_by(degron, colony) %>% tally()
```


getting the meadian values of the half lives for each degron
```{r}
...
# avg.half.all <- aic.df.allreps.all %>% 
#   mutate(degron = case_when(degron %in% c("cln2.3","cln2.2","cln2") ~ "GFP-CLN2",
#                             degron %in% c("mODC.2", "mODC")  ~ "GFP-mODC",
...
#cor with half lives and cellular features while talking to Premal
Initial GFP 
```{r}
aic.df.allreps %>% 
  filter(!(degron %in% c("cln2","mODC","stable"))) %>% 
  ...
  ggplot(.,aes(y = gfp.mean.bg.af.sub.new , x = degron, fill = colony))+
...
aic.df.allreps %>% 
  ...
  mutate(degron = case_when(degron %in% c("cln2.3","cln2.2","cln2","cln2.4") ~ "GFP-CLN2",
         ...
         colony = factor(colony, levels = c(1:12))) %>% 
  # filter(ifelse(degron == "GFP", gfp.mean.bg.af.sub.new > 1000 , gfp.mean.bg.af.sub.new > 0 )) %>% 
  ggplot(.,aes(y = area , x = degron, fill = colony))+
  geom_violin( outlier.size = 0.5)+
  facet_wrap(~degron, scales = "free")+
  # geom_jitter(position = position_jitterdodge(jitter.height = 0, jitter.width = 0.1), size = 0.2, alpha = 0.2)+
  theme_pubr()+
...
#modified fig 1d with all the reps
```{r}
...
##dist of half lives with dy.dm model for mODC and exponential model for stable and cln2
```{r}
pup1RFP.halfLives.allreps.all <- aic.df.allreps.all %>%  
  mutate(degron = case_when(degron %in% c("cln2.3","cln2.2","cln2") ~ "GFP-CLN2",
                            degron %in% c("mODC.2", "mODC")  ~ "GFP-mODC",
                            degron %in% c("stable","stable.2") ~ "GFP"),
         degron = factor(degron , levels = c("GFP-mODC","GFP-CLN2","GFP")), 
         colony = str_remove(string = colony, pattern = "Replicate ")) %>%
  ggplot(.,aes(y = log(2)/dy, x = degron, fill = colony))+
  geom_hline(data = avg.half.all, aes(yintercept = avg.t, linetype = degron))+
  geom_boxplot( outlier.size = 0.5)+
  # geom_jitter(position = position_jitterdodge(jitter.height = 0, jitter.width = 0.1), size = 0.2, alpha = 0.2)+
  theme_pubr()+
  scale_fill_brewer(name = "Replicate", palette = "Set2")+
...
```{r}
cvtHalf.plt <- cv.half %>% 
  ggplot(.,aes(y = cv.half, x = degron, color = factor(colony, levels = c(1:12))))+
  ...
  theme_pubr()+
  scale_color_brewer(name = "Replicate", palette = "Set2", guide = "none")+
  ylab("C.V. in Half-Life")+
  theme(axis.title.x = element_blank(), 
        axis.line = element_line(size = 0.1),
        axis.ticks = element_line(size = 0.2),
        text = element_text(size = 6), 
...
cvGFP.plt.grc <- cv.gfp.grc %>% 
  ggplot(.,aes(y = cv2.gfp, x = degron, fill = factor(colony, levels = c(1:12))))+
  geom_col(width = 0.4, position = "dodge")+
  # geom_point(position = position_jitter(seed = 1, width = 0.1), size = 0.5)+
  # geom_jitter(width = 0.1, size = 0.5, seed = 1)+
  # geom_crossbar(data=cv.half.median, aes(ymin = cv.half, ymax = cv.half),
  ...
  theme(axis.title.x = element_blank(), 
        axis.line = element_line(size = 0.1),
        axis.ticks = element_line(size = 0.2),
        text = element_text(size = 6), #changed from 12 to 8 for paper
        legend.position = c(0.4,0.9), 
        legend.direction = "horizontal", 
        legend.background = element_blank(), 
        legend.key.size = unit(3 ,"mm"),
...
```{r}
gfp.med <- all.exp.list %>% 
  filter(red == "pup1-rfp", treatment == "none", degron != "cln2.2") %>%
  filter(degron %in% c("cln2.3","mODC.2","stable.3")) %>% 
  filter(image.no < 32) %>% 
  mutate(degron = case_when(degron %in% c("cln2.3","cln2.2","cln2") ~ "GFP-CLN2",
                            degron %in% c("mODC.2", "mODC")  ~ "GFP-mODC",
                            degron %in% c("stable","stable.2","stable.3") ~ "GFP"),
         degron = factor(degron , levels = c("GFP-mODC","GFP-CLN2","GFP")),
         image.no = factor(image.no, levels = c(31:1))) %>%
  filter(image.no == 1) %>% 
  group_by(degron) %>% 
  summarise(med.gfp = median(gfpMeanBgAFsub))%>% 
  ungroup()
```
...
# top.sec.1 <- (wrap_elements(grid::textGrob('methods cartoon'))|wrap_elements(grid::textGrob('Model')))+
#   plot_layout(widths = c(0.7,0.3))

right.sec <- (GFPtrace.pup1Rep2022/pup1RFP.halfLives.oneReps/cvtHalf.plt)+
  plot_layout(heights = c(0.3,0.7,0.3))
...
```{r}
ggsave(plot = figGFP.timelapse, path = "~/plots/paper1/figures/fig_1/", filename = "figGFP_Timelapse.pdf", width = 6, height = 5 )
ggsave(plot = figGFP.timelapse, path = "~/plots/paper1/figures/fig_1/", filename = "figGFP_Timelapse.png", width = 6, height = 5)
```
