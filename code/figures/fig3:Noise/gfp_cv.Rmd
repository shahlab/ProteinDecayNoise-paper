...
Load the dataframe with the decay parameters and cellular attributes
```{r}
...
Experimental CV^2 in GFP intensity at each timepoint
```{r}
cv.df <- timeseries.df %>% 
  group_by(cell.id) %>% 
...
# Panel B: plotting cv in half life
```{r}
...
### mechanistic Eq CV^2(t)/CV^2(0) in GFP intensity
```{r}
...
### Calculating the CV^2(t)/CV^2(0) using the fitted values of CV0 
```{r}
...
mean.gfp.plt
```
## mean normalized gfp intensity over time
```{r}
